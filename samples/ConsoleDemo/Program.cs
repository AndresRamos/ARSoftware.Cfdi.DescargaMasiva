...
string rutaCertificadoPfx = @"C:\AR Software\CFDI Descarga Masiva\certificado.pfx";
string certificadoPassword = "12345678a";
...
{
    logger.LogError("La solicitud de autenticacion no fue exitosa. FaultCode:{0} FaultString:{1}", autenticacionResult.FaultCode,
...
if (string.IsNullOrEmpty(solicitudResult.IdSolicitud))
{
    logger.LogError("La solicitud de solicitud de descarga no fue exitosa. RequestStatusCode:{0} RequestStatusMessage:{1}",
        solicitudResult.CodEstatus, solicitudResult.Mensaje);
    throw new Exception();
}

logger.LogInformation("La solicitud de solicitud de descarga fue exitosa. RequestId:{0}", solicitudResult.IdSolicitud);
